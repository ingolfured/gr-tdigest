...
def test_cli_quantile_and_cdf(paths, dataset, expect, cfg, run_cli_fn, assert_close_fn):
...
def test_python_module_quantile_and_cdf(dataset, expect, cfg, add_pin_kw_fn, assert_close_fn):
    import gr_tdigest as td
    kwargs = dict(
        max_size=cfg["max_size"],
        scale=cfg["scale_py"],
        singleton_policy=cfg["singleton_py"],  # 'off'|'use'|'edges'
        precision=cfg["precision_py"],         # 'f64'
    )
    if cfg["singleton_py"] == "edges" and cfg["pin_per_side"] is not None:
        add_pin_kw_fn(kwargs, int(cfg["pin_per_side"]))
...
def test_polars_plugin_quantile_and_cdf(dataset, expect, cfg, add_pin_kw_fn, assert_close_fn):
    import polars as pl
    import gr_tdigest as td
    ...
    td_kwargs = dict(
        ...
        precision=cfg["precision_pl"],  # 'f64'
    )
    if cfg["singleton_pl"] == "edges" and cfg["pin_per_side"] is not None:
        add_pin_kw_fn(td_kwargs, int(cfg["pin_per_side"]))
...
def test_java_jni_quantile_and_cdf(paths, dataset, expect, cfg, tmp_path: Path, assert_close_fn):
    import textwrap
    ...
    assert classes_dir.exists(), f"Java classes not found at {classes_dir}"
...
def test_polars_precision_matrix_3d():
    ...
    P, X = 0.5, 2.0
    EXPECT_Q50, EXPECT_CDF_AT_2 = 1.5, 0.625
